...
layout(push_constant) uniform PushConstants {
    float step;  // Texel step size (1.0/height)
} pc;
...
void main() {
    ...
    for (int i = 0; i < 13; i++) {
        vec2 sampleCoord = fragTexCoord + vec2(0.0, offsets[i] * pc.step);
        ...
        if (sampleCoord.x >= 0.0 && sampleCoord.x <= 1.0 && 
            ...
            float brightness = 1.0 + luminance * 2.0;
    ...
    // Apply slight curve adjustment for final bloom look
